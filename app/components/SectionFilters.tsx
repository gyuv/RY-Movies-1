...
export default function SectionFilters({ sortOptions, extraOptions = [] }: SectionFiltersProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-4 mb-8 px-4 sm:px-6 lg:px-8">
      ...
      {/* Additional Dynamic Filters (Genre, Year, Language, Region) */}
        ...
        return (
          <div key={extra.name} className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-ink-raised border border-ink-line text-paper text-xs rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-marquee max-w-[180px] truncate"
            >
              <option value="">All {extra.label}s</option>
